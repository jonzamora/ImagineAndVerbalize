...
save_dir="${project_dir}/checkpoints/${dataset}_${train_ratio}/${method}_mixGT${graph_source_alpha}/$model_type/bs${train_batch_size}-${num_gpu}_gs${grad_step}_lr${learning_rate}_wd${weight_decay}_e${num_epoch}_seed${episodes}"
mkdir -p $save_dir

# export SLURM_NTASKS=$num_gpu
# srun --gres=gpu:8000:${num_gpu} python \
num_gpu=1
